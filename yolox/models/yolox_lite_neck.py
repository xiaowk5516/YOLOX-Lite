...
from .network_blocks import SimConv, CSPLayer, GSConv, Transpose, CAM, RepCSPLayer
...
class RepGSAttFusion(nn.Module):
    ...
    def __init__(
        ...
        self.f2_att = CAM(self.in_channels[0])
        ...
        self.p2_att = CAM(self.in_channels[0])
        # Concate(self.in_channels[0], self.in_channels[0], self.in_channels[1])
        ...
        self.p1_att = CAM(self.in_channels[1])
    ...
    def forward(self, input):
        ...
        p1_down_x = self.p1_att(p1_down_x)
...
class RepBottleSlimFusion(nn.Module):
    """
    YOLOv3 model. Darknet 53 is the default backbone of this model.
    """
    ...
    def __init__(
        ...
        # Concate(self.in_channels[0], self.in_channels[0], self.in_channels[1])
        self.f2_conv1 = Conv(self.in_channels[1] + self.in_channels[0]*2, self.in_channels[1], 1, 1) # 15
        self.f2_C3 = RepCSPLayer(self.in_channels[1], self.in_channels[1], round(12 * depth), False, act=act)  # 16
        self.f2_conv2 = Conv(self.in_channels[1], self.in_channels[0], 1, 1) # 17
        self.f2_att = CAM(self.in_channels[0])
        ...
        # Concate(self.in_channels[0], self.in_channels[1])
        self.f3_conv1 = Conv(self.in_channels[1], self.in_channels[0], 1, 1)
        self.f3_C3 = RepCSPLayer(self.in_channels[0], self.in_channels[0], round(12 * depth), False, act=act)
        ...
        # Concate(self.in_channels[0], self.in_channels[0], self.in_channels[1])
        self.p2_conv = Conv(self.in_channels[2], self.in_channels[1], 1, 1)
        self.p2_C3 = RepCSPLayer(self.in_channels[1], self.in_channels[1], round(12 * depth), False, act=act)

        # bottom-up p1
        self.p1_down = Conv(self.in_channels[1], self.in_channels[1], 3, 2)
        self.p1_att = CAM(self.in_channels[1])
        # Concate(self.in_channels[1], self.in_channels[1])
        self.p1_C3 = RepCSPLayer(self.in_channels[2], self.in_channels[2], round(12 * depth), False, act=act)
